...
This guide applies to PCssak AroDamA Free Early Access `0.4.2`; reviewed 2026-09-03.
Older releases retain their version-pinned instructions and evidence.

## 1. Use an official release

Use the public, non-draft, non-prerelease `v0.4.2` release with the 17 assets documented in
[RELEASE-ASSET-POLICY.md](RELEASE-ASSET-POLICY.md). Verify the selected version's files and check
GitHub Latest separately; this guide does not mark a fresh installation or real in-app upgrade
as passed.
...
| --- | --- | --- |
| Windows 11 Home/Pro x64 | `PCSSAK-AroDamA-Free-Early-Access-v0.4.2-Windows-x64-Setup.exe` | Primary support candidate |
| Windows 10 22H2 Home/Pro x64 | x64 installer | Limited compatibility candidate |
| Windows 10 22H2 Home/Pro x86 | `PCSSAK-AroDamA-Free-Early-Access-v0.4.2-Windows-x86-Setup.exe` | Limited compatibility candidate |
...
The `0.4.2` installers are **not Windows Authenticode-signed**. Windows may show Unknown Publisher,
...
1. If AroDamA is running, choose Quit from its tray menu. The window's X only hides the app.
...
On a new 0.4.2 installation, the selected installer language is applied once to the app. An update
...
Windows language preferences and then English. See the [0.4.2 release notes](../RELEASE-NOTES.md).
...
After you confirm and start capture, the window stays open. A short completion guide explains
copying, the actual configured shortcut, and the tray icon. Dismiss guide closes only this message,
not the app. If the tray icon is hidden, expand Windows' hidden-icons area. Closing the window with
X or Esc hides it; use the shortcut or tray icon to reopen it. First-run and renewed-consent screens
do not retain an external automatic-paste destination. Existing users keep their saved language and
consent; 0.4.2 does not request fresh consent solely for this interface change.
...
- Choose **Copy** to keep the window open, then use `Ctrl+V` in your chosen app. Text items also
  offer **Copy as plain text**. Images and files do not show a formatting-removal action.
- For **Paste into previous app**, first select the destination field and open AroDamA with its
  shortcut. This action attempts automatic input; plain-text input remains under paste options.
- Apply tags for descriptions or collections for project material. **View matching history** opens
  the full matching history, even from Pinned. Unchecking disconnects this item; deleting a
  classification name disconnects it from all items without deleting their content.
...
A normal 0.4.2 uninstall preserves history and settings. The explicit `Delete app data` option
